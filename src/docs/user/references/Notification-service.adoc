...
which will be able to subscribe to the channel.

It can either be authorized for all groups, one or more groups of the current users, or any free input.
...
The notification's message, severity and notified groups are configurable by the user.
...
- *CHANNELS:* A single channel or a comma separated list of channels that will be notified.
By default, this value is set to "All". If the user executing the workflow is a notification-service administrator, it will notify all channels. Otherwise
it notifies all channels available for the user to subscribe.
